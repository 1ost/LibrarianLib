...
class TestListScreen(name: String, val tests: List<FacadeTestEntry>): Screen(StringTextComponent(name)) {
    val itemHeight: Int = Client.fontRenderer.lineHeight + 1
    val border: Int = 3
        ...
        for(i in scrollCount until tests.size) {
            if(i == hoveredIndex) {
                fill(matrixStack,
                    ...
                    guiPos.xi + size.xi, guiPos.yi + i * itemHeight + Client.fontRenderer.lineHeight,
                    Color.gray.rgb
                )
            }

            Client.fontRenderer.draw(matrixStack, tests[i].name, guiPos.xf, guiPos.yf + i * itemHeight, Color(0, 0, 0, 0).rgb)
        }
    }
